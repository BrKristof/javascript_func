const htmlDropdown = document.getElementById('htmldropdown'); // elkérjük a htmldropdown id-jú elemet, ha ezt utána hívnánk meg akkor nem jelenítené meg a divet
...
function hideBasedOnSelected(dropdownList){
    ...
    const listValue = dropdownList.value  //a paraméter értéke

    const parentDiv = dropdownList.parentElement //az lenyíló elemek parentdivje
    const cardClasses = parentDiv.parentElement.querySelectorAll('.card') // az összes .card osztállyal rendelkező elem

    for(const c of cardClasses){

        c.classList.add('hide') // mindegyiket elrejtük
    }

    makeVisibleBasedOnId(parentDiv,listValue) // meghivjuk ezt a függvényt amivel látható lesz a kiválasztott elem
}
